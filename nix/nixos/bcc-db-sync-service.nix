...
let
  cfg = config.services.bcc-db-sync;
  self = cfg.dbSyncPkgs;
...
in {
  options = {
    services.bcc-db-sync = {
      enable = lib.mkEnableOption "enable the bcc-db-sync service";
      script = lib.mkOption {
        internal = true;
      ...
      extended = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "use bcc-db-sync-extended";
      };
      takeSnapshot = lib.mkOption {
        type = lib.types.enum [ "never" "once" "always" ];
        default = "never";
        description = ''Take snapshot before starting bcc-db-sync,
      ...
      restoreSnapshot = lib.mkOption {
        ...
        default = null;
        description = ''
          Restore a snapshot before starting bcc-db-sync,
      ...
      environment = lib.mkOption {
        ...
        default = self.bccLib.environments.${cfg.cluster};
      };
      ...
      logConfig = lib.mkOption {
        type = lib.types.attrs;
        default = self.bccLib.defaultExplorerLogConfig;
      };
      ...
      dbSyncPkgs = lib.mkOption {
        type = lib.types.attrs;
        default = import ../. {};
        defaultText = "bcc-db-sync pkgs";
        description = ''
          The bcc-db-sync packages and library that should be used.
      ...
      package = lib.mkOption {
        type = lib.types.package;
        default = if cfg.extended then self.bcc-db-sync-extended else self.bcc-db-sync;
      };
      user = lib.mkOption {
        type = lib.types.str;
        default = "cdbsync";
  ...
  config = lib.mkIf cfg.enable {
    services.bcc-db-sync = let
      exec = if cfg.extended then "${cfg.package}/bin/bcc-db-sync-extended" else "${cfg.package}/bin/bcc-db-sync";
    in {
      pgpass = builtins.toFile "pgpass" "${cfg.postgres.socketdir}:${toString cfg.postgres.port}:${cfg.postgres.database}:${cfg.postgres.user}:*";
      script = pkgs.writeShellScript "bcc-db-sync" ''
        set -euo pipefail
        ...
        ''}
          set +e
          SNAPSHOT_SCRIPT=$( (yes phrase ||:) | bcc-db-tool prepare-snapshot --state-dir ${cfg.stateDir} | tail -n 1)
          res=$?
          set -e
          if [ $res -eq 0 ]; then
        ...
        exec ${exec} \
          --config ${configFile} \
          --socket-path "$BCC_NODE_SOCKET_PATH" \
    ...
    systemd.services.bcc-db-sync = {
      ...
      path = with pkgs; [
        self.bccDbSyncHaskellPackages.bcc-db-tool.components.exes.bcc-db-tool
    ...
    assertions = [
      {
        ...
        message = "The option services.bcc-db-sync.stateDir should have ${stateDirBase} as a prefix!";
      }
    ];
  };
}
