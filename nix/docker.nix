...
#  To download and restore a snapshot include -e RESTORE_SNAPSHOT=https://update-bcc-mainnet.tbco.io/bcc-db-sync/db-sync-snapshot-schema-10-block-6014140-x86_64.tgz
#  See the latest releases for a recent snapshot https://github.com/The-Blockchain-Company/bcc-db-sync/releases
...
, dbSyncRepoName ? "tbco/bcc-db-sync"
}:
  ...
  dbSyncDockerImage = let
    ...
    entry-point = writeScriptBin "entry-point" ''
      ...
      if [[ -z "$NETWORK" ]]; then
        ...
        if [[ ! -z "''${EXTENDED}" ]] && [[ "''${EXTENDED}" == true ]]
        then
          DBSYNC=${bcc-db-sync-extended}/bin/bcc-db-sync-extended
        else
          DBSYNC=${bcc-db-sync}/bin/bcc-db-sync
        fi
