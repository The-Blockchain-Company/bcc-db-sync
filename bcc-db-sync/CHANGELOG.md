# Revision history for bcc-db-sync
...
* Note that this release requires the database to be dropped and recreated.
* Add and populate 'tx_metbccta' table.
...
* * Updates to support the schema additions, see bcc-db 2.0.0 CHANGELOG.md
...
* Add handling of Sophie era blocks to the existing handling of Cole era blocks
...
* Renamed from bcc-explorer-node to bcc-db-sync.
...
* Update to latest version of bcc-ledger, shardagnostic-network,
  shardagnostic-consensus, tbco-monitoring-framework, and bcc-shell libs.
