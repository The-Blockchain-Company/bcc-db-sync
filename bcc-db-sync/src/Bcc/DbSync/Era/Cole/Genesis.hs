...
module Godx.DbSync.Era.Cole.Genesis
...
import qualified Godx.Crypto as Crypto (Hash, fromCompactRedeemVerificationKey,
                   serializeCborHash)
...
validateGenesisDistribution tracer networkName cfg bid =
  runExceptT $ do
    ...
    case DB.word64ToGodx <$> configGenesisSupply cfg of
      ...
      Right expectedSupply ->
        when (expectedSupply /= totalSupply) $
          dbSyncNodeError  $ Text.concat
                ...
                , DB.renderGodx expectedSupply
                , " but got "
                , DB.renderGodx totalSupply
                ]
    liftIO $ do
      logInfo tracer "Initial genesis distribution present and correct"
      logInfo tracer ("Total genesis supply of Godx: " <> DB.renderGodx totalSupply)

-- -----------------------------------------------------------------------------

insertTxOuts :: (MonadBaseControl IO m, MonadIO m) => DB.BlockId -> (Cole.Address, Cole.Isaac) -> ReaderT SqlBackend m ()
insertTxOuts blkId (address, value) = do
  ...
  txId <- DB.insertTx $
            DB.Tx
              ...
              , DB.txBlockId = blkId
              , DB.txBlockIndex = 0
              , DB.txOutSum = DB.DbIsaac (Cole.unsafeGetIsaac value)
  ...
  void . DB.insertTxOut $
            DB.TxOut
              ...
              , DB.txOutValue = DB.DbIsaac (Cole.unsafeGetIsaac value)
...
configGenesisSupply :: Cole.Config -> Either Cole.IsaacError Word64
configGenesisSupply =
  fmap Cole.unsafeGetIsaac . Cole.sumIsaac . map snd . genesisTxos

genesisTxos :: Cole.Config -> [(Cole.Address, Cole.Isaac)]
genesisTxos config =
    avvmBalances <> nonAvvmBalances
  where
    avvmBalances :: [(Cole.Address, Cole.Isaac)]
    avvmBalances =
    ...
    nonAvvmBalances :: [(Cole.Address, Cole.Isaac)]
    nonAvvmBalances =
