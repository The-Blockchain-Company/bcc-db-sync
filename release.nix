...
# The project sources
{ bcc-db-sync ? { outPath = ./.; rev = "abcdef"; }

# Function arguments to pass to the project
, projectArgs ? {
    inherit sourcesOverride;
    config = { allowUnfree = false; inHydra = true; };
    gitrev = bcc-db-sync.rev;
  }
...
let
  ...
  jobs = {
    ...
    bcc-db-sync-linux = import ./nix/binary-release.nix {
      inherit pkgs project;
      platform = "linux";
      exes = filter (p: p.system == "x86_64-linux") (collect isDerivation jobs.musl64.exes);
    };
    bcc-db-sync-macos = import ./nix/binary-release.nix {
      inherit pkgs project;
      platform = "macos";
    ...
    musl64 = mapTestOnCross musl64 (packagePlatformsCross (filterProject noMusl64Build));
      ...
      [
        ...
        jobs.bcc-db-sync-linux
        jobs.bcc-db-sync-macos
