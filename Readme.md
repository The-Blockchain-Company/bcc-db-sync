...
* `bcc-db` which defines common data types and functions used by any application that needs
...
The two versions `bcc-db-sync` and `bcc-db-sync-extended` are fully compatible and use
...
The system requirements for `bcc-db-sync` (with both `db-sync` and the `node` running
